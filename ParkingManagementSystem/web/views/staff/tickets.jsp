...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h2>Danh sách Vé đỗ xe (Ca trực hiện tại)</h2>
    <a href="${pageContext.request.contextPath}/staff/tickets/checkin" class="btn btn-success">+ Check-in Xe mới</a>
</div>

<div class="card shadow-sm mb-4">
    <div class="card-body">
        <form action="${pageContext.request.contextPath}/staff/tickets" method="GET" class="row gx-3 gy-2 align-items-center">
            <div class="col-sm-4">
            ...
            <div class="col-sm-2">
                <a href="${pageContext.request.contextPath}/staff/tickets" class="btn btn-outline-secondary w-100">Bỏ lọc</a>
            </div>
        </form>
    </div>
</div>
            ...
            <tbody>
                <c:forEach var="t" items="${tickets}">
                    <tr>
                        ...
                        <td>
                            <a href="${pageContext.request.contextPath}/staff/tickets/view?id=${t.id}" class="btn btn-sm btn-outline-info">Chi tiết</a>
                            <c:if test="${t.status == 'Parking'}">
                                <a href="${pageContext.request.contextPath}/staff/tickets/checkout?ticketCode=${t.ticketCode}" class="btn btn-sm btn-outline-success ms-1">Thanh toán (Ra)</a>
                            </c:if>
                        </td>
                    </tr>
                </c:forEach>
                <c:if test="${empty tickets}">
                    <tr><td colspan="6" class="text-center text-muted">Không có dữ liệu vé trong ca của bạn.</td></tr>
                </c:if>
            </tbody>
        </table>
    </div>
